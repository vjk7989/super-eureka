...
export default function Reports({ searchParams }: { searchParams?: Record<string, string | string[] | undefined> }) {
  ...
  return <AppShell role={role}><h2 className="mb-4 text-2xl font-semibold">Reports And Exports</h2><div className="grid gap-4 md:grid-cols-2">{reports.map(([href, label]) => <Link key={href} href={withRole(href, role)} className="rounded-lg border border-border bg-card p-4 hover:bg-accent">{label}<span className="mt-2 block text-sm text-muted-foreground">CSV export prototype. PDF/Excel comes after backend wiring.</span></Link>)}</div></AppShell>;
}
